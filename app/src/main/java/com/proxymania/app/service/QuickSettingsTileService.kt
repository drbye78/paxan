...
class QuickSettingsTileService : TileService() {
            ...
            try {
                ...
                if (proxy != null) {
                    val intent = Intent(this@QuickSettingsTileService, VpnService::class.java).apply {
                        action = com.proxymania.app.service.VpnService.ACTION_CONNECT
                        putExtra(com.proxymania.app.service.VpnService.EXTRA_PROXY_HOST, proxy.host)
                        putExtra(com.proxymania.app.service.VpnService.EXTRA_PROXY_PORT, proxy.port)
                        putExtra(com.proxymania.app.service.VpnService.EXTRA_PROXY_PROTOCOL, proxy.protocol.name)
                        proxy.username?.let { putExtra(com.proxymania.app.service.VpnService.EXTRA_PROXY_USERNAME, it) }
                        proxy.password?.let { putExtra(com.proxymania.app.service.VpnService.EXTRA_PROXY_PASSWORD, it) }
                    }
    ...
    private fun disconnect() {
        val intent = Intent(this, VpnService::class.java).apply {
            action = com.proxymania.app.service.VpnService.ACTION_DISCONNECT
        }
        startService(intent)
    }
    ...
    private suspend fun getDefaultProxy(): com.proxymania.app.domain.model.Proxy? {
        return try {
