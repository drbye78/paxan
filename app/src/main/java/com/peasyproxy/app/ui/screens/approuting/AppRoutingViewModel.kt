...
class AppRoutingViewModel @Inject constructor(
    ...
    private val _mode = MutableStateFlow(com.peasyproxy.app.domain.model.SplitTunnelMode.INCLUDE) 
    val mode: StateFlow<com.peasyproxy.app.domain.model.SplitTunnelMode> = _mode.asStateFlow()
    ...
    init {
        viewModelScope.launch {
            settingsRepository.appRoutingFlow.collect { config ->
                ...
                _mode.value = when {
                    !config.isIncludeMode && config.allowBypass -> com.peasyproxy.app.domain.model.SplitTunnelMode.BYPASS
                    !config.isIncludeMode -> com.peasyproxy.app.domain.model.SplitTunnelMode.EXCLUDE
                    else -> com.peasyproxy.app.domain.model.SplitTunnelMode.INCLUDE
                }
        ...
        val appList = apps.map { appInfo ->
            AppInfo(
                ...
                isSelected = when (currentMode) {
                    com.peasyproxy.app.domain.model.SplitTunnelMode.INCLUDE,
                    com.peasyproxy.app.domain.model.SplitTunnelMode.BYPASS -> config.includedApps.contains(appInfo.packageName)
                    com.peasyproxy.app.domain.model.SplitTunnelMode.EXCLUDE,
                    com.peasyproxy.app.domain.model.SplitTunnelMode.DISABLED -> config.excludedApps.contains(appInfo.packageName)
                }
            )
        }.sortedWith(
    ...
    fun setMode(newMode: com.peasyproxy.app.domain.model.SplitTunnelMode) {
        _mode.value = newMode
        viewModelScope.launch {
            when (newMode) {
                com.peasyproxy.app.domain.model.SplitTunnelMode.INCLUDE -> {
                ...
                com.peasyproxy.app.domain.model.SplitTunnelMode.BYPASS -> {
                    settingsRepository.updateIncludeMode(false)
    ...
    fun toggleMode(isIncludeMode: Boolean) {
        setMode(if (isIncludeMode) com.peasyproxy.app.domain.model.SplitTunnelMode.INCLUDE else com.peasyproxy.app.domain.model.SplitTunnelMode.EXCLUDE)
    }
